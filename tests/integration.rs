...
fn test_ask_debug_json() {
    ...
    assert!(parsed["debug"]["units_in_result"].is_number());
}
...
fn test_add_rejects_noise_tag_prefix() {
    ...
    assert!(!output.status.success(), "noise tag must reject: {output:?}");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("task:yvck"), "stderr lacks offending tag: {stderr}");
    assert!(
